    ...
    <div className="min-h-screen" style={{ background: 'var(--bg-subtle)' }}>
      <header className="flex items-center justify-between px-4 py-4 pt-safe" style={{ background: '#0A0A0A' }}>
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)' }}>Coordinator · {user?.name}</p>
          </div>
        </div>
        <button onClick={handleLogout} className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: 'rgba(255,255,255,0.08)' }}>
      ...
      <div className="px-4 pt-4 pb-24 space-y-3">
        <div className="flex items-center justify-between mb-2">
          ...
          <button onClick={() => activeHackathon && fetchTeams(activeHackathon.id)} className="btn btn-ghost btn-icon btn-sm">
        ...
        {teams.map((team) => (
          <div key={team.id} className="card p-4 space-y-3">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg flex items-center justify-center text-white font-bold" style={{ fontSize: 14, background: '#0A0A0A' }}>
                {initials(team.name)}
              </div>
            ...
            {team.leaderPhone && <p className="font-mono text-caption">{team.leaderPhone}</p>}
            ...
            {team.participants.length > 0 && (
              <div className="flex flex-wrap gap-1.5 pt-2 border-t" style={{ borderColor: 'var(--border)' }}>
                {team.participants.map((p) => (
                  <span key={p.id} className="px-2 py-0.5 rounded-full font-medium"
                    style={{ fontSize: 11, background: p.isLeader ? '#0A0A0A' : 'var(--bg-muted)', color: p.isLeader ? 'white' : 'var(--text-secondary)' }}>
                    {p.name}
                  </span>
                ))}
              </div>
        ...
        {teams.length === 0 && (
          <div className="empty-state"><p className="text-caption">No teams assigned to you yet</p></div>
        )}
      </div>
    </div>
  );
