...
export function CheckInPage() {
  ...
  const matches = query.trim()
    ? teams.filter((t) => t.name.toLowerCase().includes(query.toLowerCase()) || t.participants.some((p) => p.name.toLowerCase().includes(query.toLowerCase()))).slice(0, 6)
    : [];
  ...
  const doCheckIn = async (team: Team) => {
    ...
    try {
      ...
      setLog((l) => [{ team: { ...team, status: 'CHECKED_IN' as TeamStatus }, success: true, msg: 'Checked in', time: new Date() }, ...l].slice(0, 20));
      toast(`✓ ${team.name}`, 'success');
    } catch (e: any) {
      setLog((l) => [{ team, success: false, msg: e.message, time: new Date() }, ...l].slice(0, 20));
      toast(e.message, 'error');
    } finally { setProcessing(false); setQuery(''); inputRef.current?.focus(); }
  };
  ...
  return (
    <div className="max-w-lg mx-auto px-5 py-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-1.5"><ScanLine className="w-4 h-4" style={{ color: 'var(--text-muted)' }} /><span className="text-caption">QR mode ready</span></div>
      </div>
      <div className="card p-5 mb-5">
        ...
        <div className="progress-bar"><div className="progress-fill progress-fill-success" style={{ width: `${pct}%` }} /></div>
      </div>
      <div className="relative mb-3">
        ...
        <input ref={inputRef} type="search" value={query} onChange={(e) => setQuery(e.target.value)}
          ...
          placeholder="Team name, member, or phone…" className="input pl-10" style={{ height: 44, fontSize: 15 }}
        ...
        <div className="card overflow-hidden mb-4">
          {matches.map((team) => (
            <button key={team.id} onClick={() => doCheckIn(team)} disabled={processing} className="table-row w-full text-left disabled:opacity-50">
              ...
              <div className="flex-1 min-w-0"><p className="font-medium truncate" style={{ fontSize: 14 }}>{team.name}</p><p className="text-caption">{team.participants.length} members</p></div>
              <div className="flex items-center gap-2 flex-shrink-0">
                <span className={cn('badge', `badge-${team.status.toLowerCase()}`)}>{team.status.replace('_', ' ')}</span>
                {team.status === 'REGISTERED' && <div className="w-7 h-7 rounded-md flex items-center justify-center" style={{ background: 'var(--success-bg)' }}><UserCheck className="w-3.5 h-3.5" style={{ color: 'var(--success)' }} /></div>}
              </div>
            </button>
          ))}
        </div>
        ...
        <div>
          ...
          <div className="card overflow-hidden">
            {log.slice(0, 10).map((entry, i) => (
              <div key={i} className="flex items-center gap-3 px-4 py-3 border-b last:border-0" style={{ borderColor: 'var(--border)', background: i === 0 ? 'var(--bg-subtle)' : 'transparent' }}>
                {entry.success ? <CheckCircle2 className="w-4 h-4 flex-shrink-0" style={{ color: 'var(--success)' }} /> : <XCircle className="w-4 h-4 flex-shrink-0" style={{ color: 'var(--danger)' }} />}
                <div className="flex-1 min-w-0"><p className="font-medium truncate" style={{ fontSize: 14 }}>{entry.team.name}</p><p style={{ fontSize: 12, color: entry.success ? 'var(--success)' : 'var(--danger)' }}>{entry.msg}</p></div>
                <span className="font-mono text-caption flex-shrink-0">{entry.time.toLocaleTimeString('en', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</span>
              </div>
            ))}
          </div>
        </div>
