...
export function CreateTeamSheet() {
  ...
  const [members, setMembers] = useState<Member[]>([
    { name: '', email: '', isLeader: true },
  ]);
  const [loading, setLoading] = useState(false);
  const close = () => setCreateTeamOpen(false);

  const addMember = () =>
    setMembers((m) => [...m, { name: '', email: '', isLeader: false }]);
  ...
  const updateMember = (
    ...
    setMembers((m) =>
      m.map((mem, idx) => (idx === i ? { ...mem, [key]: val } : mem))
    );

  const submit = async () => {
    if (!name.trim() || !activeHackathon) {
            ...
            <div className="space-y-2">
              {members.map((member, i) => (
                <div key={i} className="flex items-start gap-2">
                  <div
                    className="flex-1 space-y-1.5 p-3 rounded-xl"
                    style={{
                  ...
                  {i > 0 && (
                    <button
                      ...
                      className="btn btn-ghost btn-icon btn-sm mt-1"
                      style={{ color: 'var(--red)' }}
                    >
