...
export function CreateTeamSheet() {
  ...
  const updateMember = (i: number, key: keyof Member, val: string | boolean) =>
  ...
  const submit = async () => {
    ...
    setLoading(true);
    try {
      await createTeam(activeHackathon.id, {
        name: name.trim(),
        leaderPhone: leaderPhone || undefined,
        room: room || undefined,
        participants: members.filter((m) => m.name.trim()).map((m) => ({
          name: m.name.trim(),
      ...
      <div className="sheet animate-slide-up flex flex-col" style={{ maxHeight: '92vh' }}>
        ...
        <div className="flex items-center justify-between px-5 pb-4 border-b" style={{ borderColor: 'var(--border)' }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-bold" style={{ fontSize: 16 }}>New team</h2>
              <p className="text-caption mt-0.5">Add a team to this hackathon</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <button onClick={addMember} className="btn btn-ghost btn-sm gap-1" style={{ fontSize: 12 }}>
                ...
                <div key={i} className="flex items-start gap-2">
                  <div className="flex-1 space-y-1.5 p-3 rounded-lg" style={{ background: 'var(--bg-subtle)', border: '1px solid var(--border)' }}>
                    <div className="flex items-center gap-2">
                      {member.isLeader && (
                        <span className="px-1.5 py-0.5 rounded text-white font-semibold uppercase" style={{ fontSize: 9, letterSpacing: '0.05em', background: '#0A0A0A' }}>Leader</span>
                      )}
                      <input value={member.name} onChange={(e) => updateMember(i, 'name', e.target.value)}
                        placeholder="Member name" className="input" style={{ height: 30, fontSize: 13 }} />
                    </div>
                    <input value={member.email} onChange={(e) => updateMember(i, 'email', e.target.value)}
                      placeholder="Email (optional)" type="email" className="input" style={{ height: 30, fontSize: 13 }} />
                  </div>
                  {i > 0 && (
                    <button onClick={() => removeMember(i)} className="btn btn-ghost btn-icon btn-sm mt-1" style={{ color: 'var(--danger)' }}>
