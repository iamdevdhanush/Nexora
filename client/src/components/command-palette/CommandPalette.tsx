...
export function CommandPalette() {
  ...
  const teamActions: Action[] = teams
    ...
    .map((t) => ({
      ...
      icon: <div className="w-5 h-5 rounded flex items-center justify-center text-white font-bold" style={{ fontSize: 9, background: '#0A0A0A' }}>{t.name[0]}</div>,
      category: 'Teams',
  ...
  const grouped = all.reduce<Record<string, Action[]>>((acc, a) => { acc[a.category] = [...(acc[a.category] || []), a]; return acc; }, {});
  let gi = 0;
  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col justify-end sm:justify-center sm:items-center sm:p-4 animate-fade-in">
      ...
      <div className="relative w-full sm:max-w-md rounded-t-xl sm:rounded-xl overflow-hidden flex flex-col animate-slide-up sm:animate-scale-in"
        style={{ background: 'var(--bg)', border: '1px solid var(--border)', boxShadow: '0 20px 40px rgba(0,0,0,0.15)', maxHeight: '80vh' }}>
        <div className="flex items-center gap-3 px-4 py-3 border-b" style={{ borderColor: 'var(--border)' }}>
          <Search className="w-4 h-4 flex-shrink-0" style={{ color: 'var(--text-disabled)' }} />
          <input ref={inputRef} value={q} onChange={(e) => { setQ(e.target.value); setCursor(0); }} onKeyDown={handleKey}
            placeholder="Search teams, actions…" className="flex-1 bg-transparent outline-none" style={{ fontSize: 14 }} />
          <button className="btn btn-ghost btn-icon btn-sm" onClick={close}><X className="w-3.5 h-3.5" /></button>
        </div>
        <div className="flex-1 overflow-y-auto p-2">
          {all.length === 0 ? (
            <div className="empty-state py-8"><p className="text-caption">No results for "{q}"</p></div>
          ) : (
            Object.entries(grouped).map(([cat, actions]) => (
              <div key={cat} className="mb-2">
                ...
                {actions.map((action) => {
                  const idx = gi++;
                  return (
                    <button key={action.id} onClick={action.run} onMouseEnter={() => setCursor(idx)} className={cn('cmd-item w-full', cursor === idx && 'selected')}>
                      <div className="flex-shrink-0" style={{ color: cursor === idx ? 'white' : 'var(--text-muted)' }}>{action.icon}</div>
                      <div className="flex-1 min-w-0">
                        <p className="font-medium truncate" style={{ fontSize: 14 }}>{action.label}</p>
                        {action.sub && <p className="truncate text-caption" style={{ color: cursor === idx ? 'rgba(255,255,255,0.5)' : 'var(--text-muted)' }}>{action.sub}</p>}
                      </div>
                      <ArrowRight className="w-3.5 h-3.5 flex-shrink-0" style={{ color: cursor === idx ? 'rgba(255,255,255,0.5)' : 'var(--text-disabled)' }} />
                    </button>
                  );
                })}
              </div>
        ...
        <div className="flex items-center gap-4 px-4 py-2.5 border-t" style={{ borderColor: 'var(--border)', background: 'var(--bg-subtle)' }}>
          {[['↑↓', 'navigate'], ['↵', 'select'], ['esc', 'close']].map(([k, l]) => (
            <span key={k} className="flex items-center gap-1 text-caption">
              <kbd className="font-mono px-1.5 py-0.5 rounded" style={{ fontSize: 10, background: 'var(--bg)', border: '1px solid var(--border)' }}>{k}</kbd>{l}
            </span>
          ))}
        </div>
      </div>
