      ...
      <div className="sheet animate-slide-up flex flex-col" style={{ maxHeight: '85vh' }}>
        ...
        <div className="flex items-center justify-between px-5 pb-4 border-b" style={{ borderColor: 'var(--border)' }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-bold" style={{ fontSize: 16 }}>Invite Coordinators</h2>
          ...
          {!result ? (
            <>
              <div className="invite-box p-4">
                ...
                <p className="text-caption">Anyone with this link can join <strong>{activeHackathon?.name}</strong> as a coordinator. Links expire after the set duration.</p>
              </div>
                ...
                <div className="grid grid-cols-4 gap-2">
                  {[1, 3, 7, 14].map((d) => (
                    <button key={d} onClick={() => setExpiresInDays(d)}
                      className="py-2 rounded-lg font-semibold transition-all duration-150"
                      style={{ fontSize: 13, background: expiresInDays === d ? '#0A0A0A' : 'var(--bg-muted)', color: expiresInDays === d ? 'white' : 'var(--text-secondary)' }}>
                      {d}d
                    </button>
                  ))}
                </div>
              ...
              <label className="flex items-center justify-between p-4 rounded-lg cursor-pointer" style={{ background: 'var(--bg-subtle)', border: '1px solid var(--border)' }}>
                <div>
                  <p className="font-medium" style={{ fontSize: 14 }}>Require approval</p>
                  <p className="text-caption mt-0.5">You'll approve each coordinator before they join</p>
                </div>
            ...
            <>
              <div className="invite-box p-4 space-y-3">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <p className="font-semibold" style={{ fontSize: 14, color: 'var(--success)' }}>Invite link ready</p>
                </div>
                <div className="flex items-center gap-2">
                  <code className="flex-1 text-xs font-mono truncate px-3 py-2 rounded-md" style={{ background: 'rgba(0,0,0,0.04)', color: 'var(--text)' }}>
                    {result.url}
                  </code>
                  <button onClick={() => copy('url')} className="btn btn-secondary btn-sm flex-shrink-0">
                    {copied === 'url' ? <Check className="w-3.5 h-3.5" style={{ color: 'var(--success)' }} /> : <Copy className="w-3.5 h-3.5" />}
              ...
              <div>
                ...
                <div className="p-4 rounded-lg" style={{ background: 'var(--bg-subtle)', border: '1px solid var(--border)' }}>
                  <p style={{ fontSize: 13, lineHeight: 1.6, color: 'var(--text-secondary)' }}>{result.message}</p>
                </div>
                <button onClick={() => copy('message')} className="btn btn-secondary w-full mt-2">
                  {copied === 'message' ? <Check className="w-3.5 h-3.5" style={{ color: 'var(--success)' }} /> : <MessageSquare className="w-3.5 h-3.5" />}
                  {copied === 'message' ? 'Copied message' : 'Copy message'}
                </button>
              </div>

              <button onClick={() => setResult(null)} className="btn btn-ghost w-full" style={{ color: 'var(--text-muted)' }}>
