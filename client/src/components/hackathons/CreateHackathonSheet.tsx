...
export function CreateHackathonSheet() {
  ...
  const [form, setForm] = useState({ name: '', description: '', venue: '', startDate: '', endDate: '', maxTeams: '' });
  ...
  const submit = async () => {
    ...
    setLoading(true);
    try {
      await createHackathon({ name: form.name, description: form.description || undefined, venue: form.venue || undefined, startDate: form.startDate, endDate: form.endDate, maxTeams: form.maxTeams ? parseInt(form.maxTeams) : undefined });
    ...
    <>
      ...
      <div className="sheet animate-slide-up flex flex-col" style={{ maxHeight: '90vh' }}>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
          ...
          <textarea value={form.description} onChange={f('description')} placeholder="Description (optional)" className="input" rows={3} />
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="datetime-local" value={form.startDate} onChange={f('startDate')} className="input" style={{ fontSize: 13 }} />
            </div>
            <div>
              <p className="text-caption mb-1.5">End date *</p>
              <input type="datetime-local" value={form.endDate} onChange={f('endDate')} className="input" style={{ fontSize: 13 }} />
            </div>
          </div>
