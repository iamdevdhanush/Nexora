...
import {
  LayoutDashboard, Users, UserCheck, MessageSquare, Award,
  Zap, ChevronDown, Plus, LogOut, Loader2, Check
} from 'lucide-react';
    ...
    <aside className="sidebar flex flex-col h-screen sticky top-0">
      ...
      <div className="px-3 pt-4 pb-2">
        <button
          ...
          className={cn(
            'w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg transition-colors duration-100',
            switcherOpen ? 'bg-[var(--bg-muted)]' : 'hover:bg-[var(--bg-muted)]'
          )}
        >
          <div
          ...
          <div className="flex-1 text-left min-w-0">
            <p className="font-semibold truncate" style={{ fontSize: 13, color: 'var(--text)', lineHeight: 1.3 }}>
            ...
            {activeHackathon && (
              <p
                ...
                style={{
                  ...
                  color: activeHackathon.status === 'ACTIVE'
                    ? 'var(--success)'
          ...
          <ChevronDown
            ...
            style={{ color: 'var(--text-muted)', transform: switcherOpen ? 'rotate(180deg)' : 'rotate(0deg)' }}
          />
        </button>
        ...
        {switcherOpen && (
          <div
            ...
            style={{ borderColor: 'var(--border)', background: 'var(--bg)', boxShadow: '0 4px 12px rgba(0,0,0,0.08)' }}
          >
                ...
                hackathons.map((h) => (
                  ...
                  >
                    ...
                    {activeHackathon?.id === h.id && (
                      <Check className="w-3.5 h-3.5" style={{ color: 'var(--success)', flexShrink: 0 }} />
                    )}
                  </button>
                ))
              )}
      ...
      <div className="px-3 py-3 border-t" style={{ borderColor: 'var(--border)' }}>
        <div className="flex items-center gap-2.5 px-2.5 py-2 rounded-lg" style={{ cursor: 'default' }}>
          <div
            className="w-6 h-6 rounded-md flex items-center justify-center text-white font-bold flex-shrink-0"
            style={{ fontSize: 10, background: '#0A0A0A' }}
          >
            {initials(user?.name || 'U')}
          </div>
          <div className="flex-1 min-w-0">
            <p className="font-semibold truncate" style={{ fontSize: 12.5, color: 'var(--text)', lineHeight: 1.3 }}>
              {user?.name || 'User'}
            </p>
            <p className="truncate" style={{ fontSize: 11, color: 'var(--text-muted)' }}>
              {user?.role === 'SUPER_ADMIN' ? 'Admin' : 'Coordinator'}
            </p>
          </div>
          <button
            onClick={handleLogout}
            className="w-6 h-6 rounded flex items-center justify-center transition-colors duration-100"
