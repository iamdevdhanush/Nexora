    ...
    <aside className="sidebar flex flex-col h-screen sticky top-0">
      <div className="px-3 pt-4 pb-2">
        <button onClick={() => setSwitcherOpen((v) => !v)}
          className={cn('w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg transition-colors duration-100', switcherOpen ? 'bg-[var(--bg-muted)]' : 'hover:bg-[var(--bg-muted)]')}>
          ...
          <div className="flex-1 text-left min-w-0">
            ...
            {activeHackathon && (
              <p className="uppercase font-semibold" style={{ fontSize: 10, letterSpacing: '0.05em', color: activeHackathon.status === 'ACTIVE' ? 'var(--success)' : activeHackathon.status === 'ENDED' ? 'var(--text-disabled)' : 'var(--warning)' }}>
        ...
        {switcherOpen && (
          <div className="mt-1 rounded-lg overflow-hidden border" style={{ borderColor: 'var(--border)', background: 'var(--bg)', boxShadow: '0 4px 12px rgba(0,0,0,0.08)' }}>
            <div className="py-1 max-h-48 overflow-y-auto">
              {loading ? (
                <div className="flex items-center justify-center py-4"><Loader2 className="w-4 h-4 animate-spin" style={{ color: 'var(--text-muted)' }} /></div>
              ) : hackathons.length === 0 ? (
                <p className="text-center py-4 text-caption">No hackathons</p>
              ) : (
                hackathons.map((h) => (
                  <button key={h.id} onClick={() => { setActiveHackathon(h); setSwitcherOpen(false); }}
      ...
      <div className="px-3 py-3 border-t" style={{ borderColor: 'var(--border)' }}>
        <div className="flex items-center gap-2.5 px-2.5 py-2 rounded-lg" style={{ cursor: 'default' }}>
          <div className="w-6 h-6 rounded-md flex items-center justify-center text-white font-bold flex-shrink-0" style={{ fontSize: 10, background: '#0A0A0A' }}>
