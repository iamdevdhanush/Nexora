    ...
    <>
      <header className="sticky top-0 z-30 flex items-center justify-between px-4 pt-safe"
        ...
        <div className="flex items-center gap-1">
          <button onClick={() => setCommandOpen(true)} className="w-8 h-8 rounded-lg flex items-center justify-center transition-colors duration-100" style={{ color: 'var(--text-muted)' }}
            ...
            <div className="px-4 pb-2">
              ...
              <div className="space-y-1 overflow-y-auto" style={{ maxHeight: 260 }}>
                {hackathons.map((h) => (
                  <button key={h.id} onClick={() => { setActiveHackathon(h); setSwitcherOpen(false); }}
                    className={cn('w-full flex items-center justify-between px-3 py-3 rounded-lg transition-colors duration-100 text-left', activeHackathon?.id === h.id ? 'bg-[#0A0A0A]' : 'hover:bg-[var(--bg-subtle)]')}
                    style={{ border: '1px solid var(--border)' }}>
                    <div>
                      <p className="font-semibold" style={{ fontSize: 14, color: activeHackathon?.id === h.id ? 'white' : 'var(--text)' }}>{h.name}</p>
                      <p style={{ fontSize: 12, color: activeHackathon?.id === h.id ? 'rgba(255,255,255,0.5)' : 'var(--text-muted)', marginTop: 1 }}>
                        {h._count?.teams ?? 0} teams · {h.status}
                      </p>
                    </div>
                    {activeHackathon?.id === h.id && (
                      <div className="px-2 py-0.5 rounded-full font-semibold uppercase" style={{ fontSize: 10, letterSpacing: '0.05em', background: 'rgba(255,255,255,0.15)', color: 'white' }}>Active</div>
                    )}
                  </button>
                ))}
              </div>
              {isAdmin && (
                <button onClick={() => { setSwitcherOpen(false); setCreateHackathonOpen(true); }}
                  ...
                  style={{ border: '1px dashed var(--border)', fontSize: 14, fontWeight: 500, color: 'var(--text-muted)' }}>
