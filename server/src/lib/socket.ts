...
export const setupSocketHandlers = (io: Server) => {
  io.on('connection', (socket) => {
    ...
    socket.on('join:hackathon', (hackathonId: string) => { socket.join(`hackathon:${hackathonId}`); });
    socket.on('leave:hackathon', (hackathonId: string) => { socket.leave(`hackathon:${hackathonId}`); });
