...
messagesRouter.post('/broadcast', requireAdmin, async (req: AuthRequest, res) => {
  ...
  const teams = await prisma.team.findMany({
    where: teamIds === 'all' ? { hackathonId } : { hackathonId, id: { in: teamIds } },
    select: { id: true, name: true, leaderPhone: true },
  });

  const message = await prisma.message.create({
    data: {
      content, channel, status: 'QUEUED',
      recipientType: teamIds === 'all' ? 'ALL' : 'SELECTED',
...
messagesRouter.post('/:id/retry', requireAdmin, async (req: AuthRequest, res) => {
  const message = await prisma.message.findUnique({
    ...
    include: { recipients: { where: { status: 'FAILED' }, include: { team: true } } },
  });
